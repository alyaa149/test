...
Future<void> _fetchUserName() async {
  try {
    ...
    DocumentSnapshot workerSnapshot = await FirebaseFirestore.instance.collection('workers').doc(widget.workerId).get();

    // Extract the first name and last name from the document
    String firstName = workerSnapshot['First Name']; // Replace 'First Name' with the actual field name for first name
    String lastName = workerSnapshot['Last Name']; // Replace 'Last Name' with the actual field name for last name

    // Concatenate first name and last name
    String fullName = '$firstName $lastName'; // Concatenate first name and last name

    setState(() {
      _workerName = fullName; // Update the state variable with the worker's full name
    });
  } catch (error) {
    // Handle any errors
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Row(
          children: [
           Text(
               _workerName.isNotEmpty ? _workerName : 'Worker.Name', // Display the worker's name if available, otherwise fallback to 'Worker.Name'
                        ...
                        return ListTile(
                          title: Row(
                            ...
                            children: [
                              ...
                              SizedBox(width: isMe ? 8.0 : 0.0), // Add spacing for user messages
                              Flexible(
                                child: Align(
  ...
  Stream<QuerySnapshot<Object?>> _mergeMessageStreams() {
    final customerStream = _userMessagesCollection.orderBy('timestamp', descending: true).snapshots();
